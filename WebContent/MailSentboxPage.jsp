...
<!-- Central Modal Small -->
  <div id="page-wrapper" >  
                ...
                <div class="col-md-12 col-sm-12 col-xs-12 FormSides">
                   <div class="col-md-12" >
                       <div class="btn-group" role="group" aria-label="Basic example" style="margin-bottom: 10px;">
                        <a href="MailInboxPage.jsp" class="btn btn-sm btn-info" ><b>Inbox</b></a>
                            <a href="MailSentboxPage.jsp" class="btn btn-sm btn-primary" ><b>Sentbox</b></a>
                    ...
                    <div class="col-md-5 col-sm-12 col-xs-12" id="l" >
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12 " id="left">
                            ...
                            <h5 class="FormHeader" styles="margin : 0px"><strong>Sentbox</strong></h5>
                            <%   
                            	int SelectedID = 0;
                           	    CommonUtil common = new CommonUtil();
                            	IMailService mailService = new MailServiceImpl();
                            	IEmployeeService employeeService = new EmployeeServiceImpl();
                            	ArrayList<Mail> arrayList  = mailService.viewMails(login.getEmpId(), CommonConstants.ID_VIEW_ALL_SEND_MAILS );
                            	
                            	if( request.getAttribute("MessageID") != null ){
                                	SelectedID = Integer.parseInt( request.getAttribute("MessageID").toString() );
                            	}else{
                            		if( !arrayList.isEmpty() )
                            		SelectedID = arrayList.get(0).getMailId();
                            	}
                            	if( !arrayList.isEmpty() ){
                            	for (Mail mail : arrayList){
                            		Employee employee = employeeService.getEmployeeDetails(mail.getReceiverId());
                            		
                            %>
                            <form action="ViewMail" method="POST" id="<%=mail.getMailId() %>" >
                            	<input type="hidden" name="mailId" value="<%=mail.getMailId() %>" >
                            	<input type="hidden" name="MessageType" value="Sentbox" >
                             <a href="javascript:{}" onclick="document.getElementById('<%=mail.getMailId() %>').submit();">
	                            <div class="col-xs-12 messageBox"  id="<%=common.messageType(mail.getMailId(), SelectedID ) %>"  >
	                            <div class="col-xs-1"  >
	                        ...
	                        <div class="col-xs-11" >
	                               <h6  class = "senderName" ><%=employee.getFirstName() + " " + employee.getLastName()  %>
	                               		<%if(mail.getRead() == 1 ){ %> <i class="fas fa-check-double"></i> <%} %>
                       ...
                       <% 
                            ...
                            Employee employee;
                         	if( request.getAttribute("MessageID") != null ){
                           	...
                           	{ 	
                           		mail = arrayList.get(0);
                           	}
                         		
                            	employee = employeeService.getEmployeeDetails(mail.getReceiverId() );   	%>
                           ...
                           <div class="col-xs-12" >
                                ...
                                <h5><%=employee.getFirstName() + " " + employee.getLastName()  %>
                                		<%if(mail.getRead() == 1 ){ %> <i class="fas fa-check-double"></i> <%} %>
                                <br>
                                ...
                                <p><%=mail.getMessage() %></p>
                                <%if(mail.getRead() == 1 ){ %>
                                <h6>Read <%=mail.getReadTime() %> <%=mail.getReadDate() %></h6>
                                <%} %>
                                <a href="MailSentboxPage.jsp?DeleteID=<%=mail.getMailId()%>">
    ...
    $(document).ready(function(){
     ...
     <% 	
    	if( request.getAttribute("MessageSent") != null ){ 
    		 boolean sent = Boolean.parseBoolean( request.getAttribute("MessageSent").toString() );  
...
<div class="modal fade" id="mailModal" aria-hidden="true">
	  <div class="modal-dialog modal-md" role="document">
      <div class="modal-header " style='border-bottom : 0px !important; padding : 5px;
      <%=(sent == false) ? "background-color:red !important;" : "" %>'>
        <h4 class="modal-title w-100 text-center" id="myModalLabel">
        <%=(sent == true) ? "Message Sent SuccessFully !" : "Message Sent UnsuccessFull !" %></h4>
